...
export class Map implements AfterViewInit {
  ...
  private map: any;
  private apiKey =
    'eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjAwYWVlN2YxZWVmYTQzNDc5MTRiOTM4NGNiNDcyYjBkIiwiaCI6Im11cm11cjY0In0=';
  ...
  private initMap() {
    this.map = L.map('map').setView([25.033, 121.5654], 13);
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        coords.forEach((point, index) => {
          ...
          const lng = point[0];
          const text =
            index === 0 ? '起點' : index === coords.length - 1 ? '終點' : `中途點 ${index}`;
